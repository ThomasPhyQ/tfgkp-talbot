...
###########################################
### Gate fidelity at Half Talbot length ###
###########################################
...
def T_op(x, gkp_ref):
    return 1/np.sqrt(2)*(np.dot(gkp_ref[0,:].conj(),x)*gkp_ref[0,:] - 1j*np.dot(gkp_ref[0,:].conj(),x)*gkp_ref[1,:] + np.dot(gkp_ref[1,:].conj(),x)*gkp_ref[1,:] - 1j*np.dot(gkp_ref[1,:].conj(), x)*gkp_ref[0,:])

def T_op_conj(x, gkp_ref):
    return 1/np.sqrt(2)*(np.dot(gkp_ref[0,:].conj(),x)*gkp_ref[0,:] + 1j*np.dot(gkp_ref[1,:].conj(),x)*gkp_ref[0,:] + np.dot(gkp_ref[1,:].conj(),x)*gkp_ref[1,:] + 1j*np.dot(gkp_ref[0,:].conj(), x)*gkp_ref[1,:])
...
for bw in range(n_param):
    ...
    for lw in range(n_param):  
        ...
        disp = np.exp(-1j*beta_T/2*freq_vector**2)
        ...
        #TT
        T_T_basis_0 = T_op_conj(T_basis_0,gkp_ref)
        T_T_basis_1 = T_op_conj(T_basis_1,gkp_ref)
...
with open("data/gate_fidelity_half_bw_lw.pkl","wb") as f:
    pic.dump(dict_gkp,f)
